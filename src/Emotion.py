...
with open("C:/Users/riadm/Desktop/BertTurkModelFinal/fine_tuned_results_turkish/label_encoder.pkl", "rb") as f:
...
def speak_with_emotion(sentence):
    ...
    payload = {
        ...
        "voice_settings": {"stability": 0.48, "similarity_boost": 0.67}
    }
...
def analyze_text_and_generate_audio(text, filename):
    ...
    for sentence in sentences:
        if sentence.strip():
            ...
            # Wrap the sentence with <emotion> tags
            modified_sentence = f"<{predicted_emotion}> {sentence.strip()} </{predicted_emotion}>"
            
            audio_stream = speak_with_emotion(modified_sentence)
            audio = AudioSegment.from_file(audio_stream, format="mp3")
            combined_audio += audio
            emotion_results.append((modified_sentence, predicted_emotion, confidence))
